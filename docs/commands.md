...
| `bladewright:uninstall` | Take Bladewright out: the `bw_` tables, accounts, storage. Asks whether the uploaded files go too, then wants the site's name typed. Finish with `composer remove bladewright/builder` |
...
| Command | What it does |
|---|---|
| `bladewright:layouts` | List them: name, type, the typeface they lend. `--search=` narrows |
| `bladewright:layouts --create=site --type=sidebar` | A new frame. Types: header (default) or sidebar. **What the CSS is written in is the site's own word**, said once at install, so a frame is born wearing it. The recipe's job ends at birth — the frame is the site's own from then on |
...
| Command | What it does |
|---|---|
| `bladewright:components` | List them: name, type, how the contents stand, how many blocks. `--search=` narrows |
| `bladewright:components --create=hero --type=section` | A new component. Types: section / article / header / footer / nav / table / figure / form / field. `--layout=` says how the contents stand: stack (default), grid, or row |
